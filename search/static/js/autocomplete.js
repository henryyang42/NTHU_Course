...
NTHUCourse.Autocomplete = function(input) {
  ...
  */
  ...
  this.box = $('<span class="NTHUCourse-autocomplete"></span>')
}
...
NTHUCourse.Autocomplete.prototype.inputBlur = function(e) {
  window.setTimeout($.proxy(this.hide, this), this.hideAfter);
}

NTHUCourse.Autocomplete.prototype.inputClick = function(e) {
  if (this.value.length >= this.minimumCharacters)
    this.show();
}

// When mouse enters the box:
NTHUCourse.Autocomplete.prototype.boxMouseenter = function(e) {
  ...
  this.input.trigger('dehilightChoice', [current, this]);

  // ... and then sent the hilight signal for the choice.
  this.input.trigger('hilightChoice', [$(e.currentTarget), this]);
}

// When mouse leaves the box:
NTHUCourse.Autocomplete.prototype.boxMouseleave = function(e) {
  // Send dehilightChoice when the mouse leaves a choice.
  this.input.trigger('dehilightChoice', [this.box.find('.' + this.hilightClass), this]);
}

// When mouse clicks in the box:
NTHUCourse.Autocomplete.prototype.boxClick = function(e) {
  var current = this.box.find('.' + this.hilightClass);
...
// shown.
NTHUCourse.Autocomplete.prototype.show = function(html) {
...
NTHUCourse.Autocomplete.prototype.hide = function() {
  this.box.hide();
}

// This function is in charge of hilighting the right result from keyboard
// navigation.
NTHUCourse.Autocomplete.prototype.move = function(e) {
  ...
  this.input.trigger('hilightChoice', [target, this]);
}

// Calculate and set the outer container's absolute positionning.
NTHUCourse.Autocomplete.prototype.fixPosition = function() {
...
inputs. It is named 'NTHUCourseAutocomplete' rather than just 'autocomplete'
...
function that should override the Autocomplete builtin. For example:
  ...
  $('input#your-autocomplete').NTHUCourseAutocomplete({
      url: '/some/url/',
      hide: function() {
          this.outerContainer
      },
...
Also, it implements a simple identity map, which means that:
  ...
  $('input#your-autocomplete').NTHUCourseAutocomplete().data = {
      newData: $('#foo').val(),
  }

To destroy an autocomplete, call NTHUCourseAutocomplete('destroy').
*/
